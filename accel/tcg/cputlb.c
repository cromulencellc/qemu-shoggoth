                         ...
                         bool recheck, MMUAccessType access_type, int size)
{
    ...
    // We'll only get read reads dispatched to devices using this path.
    if(is_memread_instrumentation_enabled()){
        notify_read_memory(cpu, mr_offset, addr, (uint8_t*)&val, size);
    }
                      ...
                      uintptr_t retaddr, bool recheck, int size)
{
    ...
    // This one is slightly different. We'll only get memory writes
    // dispatched to devices using this path. This isn't good enough
    // for doing Rapid Analysis RAM Deltas but is good for callbacks.
    if(is_memwrite_instrumentation_enabled()){
        notify_write_memory(cpu, mr_offset, addr, (uint8_t*)&val, size);
    }
