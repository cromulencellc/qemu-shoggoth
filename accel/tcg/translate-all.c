                              ...
                              uint32_t flags, int cflags)
{
 ...
 buffer_overflow:
    ...
    {
        sigjmp_buf *error_return_env = tcg_get_error_return_env();
        if (sigsetjmp(*error_return_env, 1) == 1)
        {
            rapid_analysis_set_error(ERROR_STATE_PROCESSOR_OP, pc, "Invalid opcode");
            cpu_loop_exit(cpu);
        }
    }
    ...
    if( is_rapid_analysis_active() ){
        RSaveTree *rst = rapid_analysis_get_instance(tcg_ctx->cpu);
        RSaveTreeClass *rst_class = RSAVE_TREE_GET_CLASS(rst);
        rst_class->prepare_tb(rst, tb);
    }
