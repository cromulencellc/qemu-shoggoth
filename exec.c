...
static void breakpoint_invalidate(CPUState *cpu, target_ulong pc)
{
    ...
    if (phys != -1) {
        ...
        tb_flush(cpu); // TODO ARC otherwise we're leaving stale BKPTs everywhere
    }
}
#endif
...
/* Called from RCU critical section */
static RAMBlock *qemu_get_ram_block_nofault(ram_addr_t addr)
{
    RAMBlock *block;
...
static void ram_block_add(RAMBlock *new_block, Error **errp, bool shared)
{
    ...
    new_block->max_pages = 0;
    new_block->rsave_flags = 0;
    new_block->rsave_l1_hashes = NULL;
    new_block->rsave_l2_hashes = NULL;
 ...
 */
void *qemu_map_ram_ptr_nofault(RAMBlock *ram_block, ram_addr_t addr, RAMBlock **dest_block)
{
    RAMBlock *block = ram_block;

    if (block == NULL) {
        block = qemu_get_ram_block_nofault(addr);
        if (block == NULL) {
            return NULL;
        }
        addr -= block->offset;
                               ...
                               uint64_t val, unsigned size)
{
    ...
    stn_p(qemu_map_ram_ptr_nofault(NULL, ram_addr, &block), size, val);
                                           ...
                                           hwaddr l, MemoryRegion *mr)
{
    ...
    for (;;) {
        if (!memory_access_is_direct(mr, true)) {
            ...
            /* RAM case */
            // Catch direct writes as a result of DMA.
            if( is_rapid_analysis_active() ){
                // Perform the translation to memory region offset.
                rapid_analysis_mark_ram_dirty(addr1, addr1 + len);
            }

            if(is_memwrite_instrumentation_enabled()){
                notify_write_memory(current_cpu, addr1, RAM_ADDR_INVALID, buf, len);
            }
    ...
    for (;;) {
        if (!memory_access_is_direct(mr, false)) {
            ...
            if(is_memread_instrumentation_enabled()){
                notify_read_memory(current_cpu, addr1, RAM_ADDR_INVALID, buf, len);
            }
        }
...
void rapid_analysis_mark_ram_clean(hwaddr start, hwaddr end)
{
    ...
    hwaddr dirty_addr = start;
    qemu_map_ram_ptr_nofault(NULL, dirty_addr, &block);
    while(block && dirty_addr < end) {
        hwaddr start_page = (dirty_addr - block->offset) >> TARGET_PAGE_BITS;
        hwaddr end_page = MIN(end - block->offset, block->used_length) >> TARGET_PAGE_BITS;
        if( block->rsave_flags ) {
            for(hwaddr dirty_page = start_page; dirty_page <= end_page; dirty_page++) {
                block->rsave_flags[dirty_page] &= ~RSAVE_LAYER2_DIRTY;
                block->rsave_flags[dirty_page >> RSAVE_LAYER1_BANK_BITS] &= ~RSAVE_LAYER1_DIRTY;
            }
        }
        dirty_addr = MIN(end, dirty_addr + block->used_length);
        qemu_map_ram_ptr_nofault(NULL, dirty_addr, &block);
    }
}
...
void rapid_analysis_mark_vram_dirty(CPUState *cpu, hwaddr start, hwaddr end)
{
    ...
    while (addr < end) {
        ...
        phys_addr += (addr & ~TARGET_PAGE_MASK);
        rapid_analysis_mark_ram_dirty(phys_addr, phys_addr + l);
        addr += l;
    }
}
