...
void hmp_rapid_load(Monitor *mon, const QDict *qdict)
{
    Error *err = NULL;
    int saved_vm_running = runstate_is_running();
    ...
    if (!filename) {
        error_report("No rapid load file specified");
        error_printf("Use file=[*.rsave] to specify a rapid analysis file\n");
        return;
    }

    vm_stop(RUN_STATE_RESTORE_VM);

    if (load_snapshot_rsave(filename, NULL, &err) == 0 && saved_vm_running) {
        vm_start();
    }
    hmp_handle_error(mon, &err);
}
...
void hmp_os(Monitor *mon, const QDict* qdict)
{
    ...
    {
        OSHandler *os_handler = oshandler_init(qemu_get_cpu(0), os_type);
        if (!os_handler || !is_oshandler_active()){
            error_setg(&err, "OShandler not created");
        }else{
            monitor_printf(mon, "Found OS %s!\n", object_get_typename(OBJECT(os_handler)));
        }
    }
...
void hmp_set_pid(Monitor* mon, const QDict* qdict)
{
    uint64_t pid = 0;
    Error *err = NULL;
    pid = qdict_get_try_int(qdict, "pid_num", -1);
...
void hmp_unset_pid(Monitor* mon, const QDict* qdict)
{
    Error *err = NULL;
    if(is_oshandler_active())
    {
        OSHandler *os_handler = oshandler_get_instance();
        OSHANDLER_GET_CLASS(os_handler)->release_ospid(os_handler, os_pid);
        os_pid = NULL_PID;
    }
    hmp_handle_error(mon, &err);
}

void hmp_exec(Monitor *mon, const QDict *qdict)
{
    Error *err = NULL;
    ...
    qemu_command_send(cmd);

    hmp_handle_error(mon, &err);
}

void hmp_get_process_list(Monitor* mon, const QDict* qdict)
{
    if(!is_oshandler_active())
    {
        error_printf("No os handler active. Please use \"os\" to activate one.\n");
    ...
    ProcessInfo *pi = os_cc->get_processinfo_by_ospid(os_handler, os_pid);

    os_cc->print_process_list(os_handler, pi);
}

void hmp_bp(Monitor *mon, const QDict *qdict)
{
    uint64_t bp_addr = 0;
    const char* addr_str = qdict_get_try_str(qdict, "address");

    if(!addr_str)
    {
        error_printf("Please specify an address for the breakpoint.\n");
        return;
    }

    if( runstate_is_running() ){
        error_printf("Please \"stop\" before setting breakpoints.\n");
        return;
    }
    ...
    if(addr_str[0] == 'p')
    {
        bp_addr = strtoull(&addr_str[1], NULL, 0);

        if( !cl->set_breakpoint(os_handler, bp_addr, 1, OS_BREAKPOINT_SW, NULL_PID) ) {
            error_printf("Unable to set breakpoint on address %"PRIx64" for physical memory. Maybe try \"bphw\"?\n", bp_addr);
        }
    }
    else
    {
        OSBreakpoint *new_bp = NULL;
        bp_addr = strtoull(addr_str, NULL, 0);

        new_bp = cl->set_breakpoint(os_handler, bp_addr, 1, OS_BREAKPOINT_SW, os_pid);

        if( !new_bp ) {
            error_printf("Unable to set breakpoint on address %"PRIx64" for set pid. Maybe try \"bphw\"?\n", bp_addr);
        }else{
            monitor_printf(mon, "Set breakpoint id %lu at address %"PRIx64"\n", new_bp->id, new_bp->addr);
        }
    }
}
...
void hmp_bphw(Monitor *mon, const QDict *qdict)
{
    ...
    if(addr_str[0] == 'p')
    {
        ...
        if( !cl->set_breakpoint(os_handler, bp_addr, 1, OS_BREAKPOINT_HW, NULL_PID) ) {
            error_printf("Unable to set breakpoint on address %"PRIx64" for physical memory. Did you exceed maximum breakpoints?\n", bp_addr);
        }
    }
    else
    {
        ...
        new_bp = cl->set_breakpoint(os_handler, bp_addr, 1, OS_BREAKPOINT_HW, os_pid);

        if( !new_bp ) {
            error_printf("Unable to set hardware breakpoint on address %"PRIx64" for set pid. Did you exceed maximum breakpoints?\n", bp_addr);
        }else{
            monitor_printf(mon, "Set hardware breakpoint id %lu at address %"PRIx64"\n", new_bp->id, new_bp->addr);
        }
    }
}
...
void hmp_bplist(Monitor *mon, const QDict *qdict)
{
    ...
    {
        OSHandler *os_handler = oshandler_get_instance();
        QList *bp_list = OSHANDLER_GET_CLASS(os_handler)->get_breakpoints(os_handler);

        monitor_printf(mon, "ID\tAddress\n");

        // Inc the refcount for the qpointers and assign to the free pool.
        QLIST_FOREACH_ENTRY(bp_list, e){
            QPointer *qptr = qobject_to(QPointer, qlist_entry_obj(e));
            OSBreakpoint *bp = qpointer_get_pointer(qptr);
            ...
            CPU_FOREACH(cpu)
            {
                ...
                if( cpu_class->get_pc ){
                    if( cpu_class->get_pc(cpu) == bp->addr ){
                        active=true;
                    }
                }
            }
...
{
    const char* bp_str = qdict_get_try_str(qdict, "bpid");

    if(!bp_str){
        error_printf("Remove breakpoint needs an integer for the breakpoint id to remove.\n");
        return;
    }

    if( runstate_is_running() ){
        error_printf("Please \"stop\" before removing breakpoints.\n");
        return;
    }
    ...
    if( !strncmp(bp_str, "all", 3) ){
        cl->remove_all_breakpoints(os_handler);
    }else{
        char *parsed = NULL;
        uint64_t bpid = strtoull(bp_str, &parsed, 10);

        if(bp_str == parsed)
        {
            error_printf("%s is not a valid format for a breakpoint id\n", bp_str);
        }
        else
        {
            OSBreakpoint *bp = cl->get_breakpoint(os_handler, bpid);
            if(bp) {
                cl->remove_breakpoint(os_handler, bp);
            }else{
                error_printf("Could not find breakpoint with id %s\n", bp_str);
            }
        }
    }
}
...
void hmp_rapid_save(Monitor *mon, const QDict *qdict)
{
    ...
    const char *filename = qdict_get_try_str(qdict, "file");
    if (!filename) {
        monitor_printf(mon, "No rapid save file specified so I generated one.\n"
