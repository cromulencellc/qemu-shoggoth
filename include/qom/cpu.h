...
typedef struct CPUClass {
    ...
    void (*init_external_register_control)(CPUState *cpu);
    void    (*set_register_data)(CPUState *cpu, uint8_t id, uint8_t size, const uint8_t *data);
    uint8_t (*get_register_data)(CPUState *cpu, uint8_t id, uint8_t **data);
    void    (*get_register_list)(CPUState *cpu, RegisterList *regs);
...
/**
 * cpu_is_waiting_for_work:
 * @cpu: The CPU to check
 * 
 * Checks if the CPU is waiting for RSave work.
 * 
 * Returns: %true if run state is waiting for work;
 * %false otherwise.
 */
