...
static void rollover_input(void)
{
   input[0]++;
            ...
            case JOB_REPORT_EXCEPTION:
                {
                    ...
                    if(ee->exception_mask & X86_SEGMENT_FAULT){
                        printf("Found a segfault!\n");
                        stop = 1;
                    }
                }
                break;
...
int main(int argc, char *argv[])
{
    ...
    string_to_hash("2cfa4aafd5904143d61fbf795b215da016f70cf5", dummy);

    printf("Waiting for rapid analysis session on port %d...\n", portno);
    ...
    while(!stop){
        ...
        out = racomms_create_job_add_msg(1, job_id, dummy, 0);
        out = racomms_msg_job_add_put_MemorySetup(out, 0x7fffffffe580, sizeof(input), input, MEMORY_VIRTUAL);
