...
DEF("display", HAS_ARG, QEMU_OPTION_display,
    ...
    "-display plugin,plugin-name=<plugin-type-str>\n"
    "-display none\n"
...
DEF("rapidanalysis", HAS_ARG, QEMU_OPTION_rapidanalysis, \
    "-rapidanalysis [file=rsave][,istep=count][,ilimit=limit][,ctrl=ip:port][,hash=sha1sum][,os=osname][,process=id][,msgsz_limit=maxsize][,timeout=to]\n" \
    "                Load a rsave file and start the emulator in analysis mode\n",
    QEMU_ARCH_ALL)
STEXI
@item -rapidanalysis @var{file}[,@var{option}[,...]]
@findex -rapidanalysis
Load an rsave file at @var{file} and starts the emulator in rapid analysis mode (@code{rloadvm} in monitor)

@option{-rapidanalysis} accepts the following options:

@table @option

@item file=@var{file}

This option defines which rsave image (@pxref{disk_images}) to use with
this drive. If the filename contains comma, you must double it
(for instance, "file=my,,file" to use file "my,file").

@item connect=@var{connect}

Sets the IP:PORT for connecting to the controller that supplies jobs requests for rapid analysis mode.

@item listen=@var{listen}

Sets the IP:PORT for listening to receive jobs requests from the controller for rapid analysis mode.

@item mode=@var{mode}

Use a preset mode that defines a collection of options, such as "user" (see ra.c).

@item istep=@var{istep}

The number of instructions to execute before taking an incremental snapshot.
Set to 0 for the number of instructions that result in the best performance.

@item ilimit=@var{ilimit}

The number of instructions to execute for an analysis session.
Set to 0 for an unlimited number of instructions.

@item hash=@var{hash}

The sha1sum of the state to use for initializing the analysis session.

@item notrace=@var{notrace}

Skips collecting a system trace.

@item notree=@var{notree}

Skips the building of the tree, inherently also skips saving state.

@item nosave=@var{nosave}

Do no automatically save final states back to the vmstate file.

@item noblocks=@var{noblocks}

Skips dumping disk data to the blocks file

@item process=@var{process}

Target the specified process when doing analysis.

@item os=@var{os}

Load the specified OS introspection module.

@item chnl_pool=@var{chnl_pool}

Initial size of global memory pool for memory channel.

@item chnl_limit=@var{chnl_limit}

Size limit for the global memory channel pool. Use zero for no limit.

@item msg_limit=@var{msg_limit}

Puts an upper bound on the size of an outgoing message.

@item ints=@var{ints}

Enable or disable interrupts. Disables some CPU/OS functions such as IO and task swapping.

@item ref_pool=@var{ref_pool}

Size of global memory pool for the reference cache.

@item timeout=@var{timeout}

Sets a timeout to interrupt an RA job that has gone on for too long.

@item none

RA is initialized and the specified hash is loaded and executed. If no hash is
specified then the first hash in the rsave is used. Load subsequent sessions with
@code{rloadvm} in monitor.

@end table

ETEXI

DEF("ra", HAS_ARG, QEMU_OPTION_ra, "", QEMU_ARCH_ALL)

DEF("plugin", HAS_ARG, QEMU_OPTION_plugin, \
    "-plugin module=[filename,arg1,...,argn]\n" \
    "        Load a plugin from the given file\n",
    QEMU_ARCH_ALL)
STEXI
@item -plugin @var{module}[,@var{arg}][,@var{arg}][,...]
@findex -plugin
Load a plugin file and passes args all parsed from @var{module}

@item module=@var{module}

This option gives the name of the plugin to load and
arguments to that plugin as a comma seperated list. The
module should be in a subdirectory called plugins.

@item arg=@var{arg}

This is an argument to the plugin.

ETEXI

DEF("plugins", HAS_ARG, QEMU_OPTION_plugins, \
    "-plugins [,module=filename][,...]\n" \
    "        Load a list of plugins from the given files\n",
    QEMU_ARCH_ALL)
STEXI
@item -plugins @var{module}[,@var{module}][,@var{module}][,...]
@findex -plugins
Loads each @var{module} as a plugin with no args

@item module=@var{module}

This is a comma separated list of plugins to load.
There will be no args passed to these plugins.
