...
def gen_registry(registry, prefix, initialize):
    ret = mcgen('''
...
''',
                ...
                initialize="QTAILQ_INIT(cmds);" if initialize else "")
    ret += registry
    ret += mcgen('''
}
''')
...
class QAPISchemaGenCommandVisitor(QAPISchemaModularCVisitor):
    ...
    def __init__(self, prefix, suplemental):
        QAPISchemaModularCVisitor.__init__(
            self, prefix, 'qapi-commands',
            ' * Schema-defined QAPI/QMP commands', __doc__)
        self._regy = QAPIGenCCode(None)
        self._visited_ret_types = {}
        self._init_cmd_list = not suplemental
...
''',
        ...
        genc.add(gen_registry(self._regy.get_content(), self._prefix, self._init_cmd_list))
...
def gen_commands(schema, output_dir, prefix, suplemental):
    vis = QAPISchemaGenCommandVisitor(prefix, suplemental)
    schema.visit(vis)
