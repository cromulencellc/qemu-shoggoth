...
{
    qiov->iov = g_new0(struct iovec, alloc_hint);
    qiov->niov = 0;
...
void qemu_iovec_resize(QEMUIOVector *qiov, int new_size)
{
    assert(qiov->nalloc != -1);

    if( new_size > qiov->nalloc ){
        qiov->iov = g_renew(struct iovec, qiov->iov, new_size);
        qiov->nalloc = new_size;
    }
}
