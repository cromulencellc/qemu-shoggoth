...
__thread QemuThread *current_thread = NULL;
...
static void *qemu_thread_start(void *arg)
{
                       ...
                       void *arg, int mode)
{
    ...
    thread->base.thread_data = data;

    err = pthread_create(&thread->thread, &attr,
                         qemu_thread_start, thread);
...
void qemu_thread_exit(void *retval)
{
    ...
    if (current_thread->base.thread_data)
    {
        delete_thread = current_thread->base.thread_data->delete;
        g_free(current_thread->base.thread_data->name);
        g_free(current_thread->base.thread_data);
        current_thread->base.thread_data = NULL;
    }
    if (delete_thread)
        g_free(current_thread);
    pthread_exit( retval );
}
...
void *qemu_thread_join(QemuThread *thread)
{
    ...
    if (thread->base.thread_data &&
        thread->base.thread_data->mode == QEMU_THREAD_DETACHED) {
        return NULL;
    }
...
{
    QemuThreadData *data = g_new0(QemuThreadData, 1);
    data->start_routine = NULL;
    data->name = g_strdup(name);
...
{
    return thread->base.thread_data->name;
}
