...
{
    if (current_thread && current_thread->base->thread_data) {
        notifier_list_add(&current_thread->base->thread_data->exit, notifier);
    } else {
...
static unsigned __stdcall win32_start_routine(void *args)
{
    QemuThread *this_thread = (QemuThread*)args;
...
{
    QemuThreadData *data = current_thread->base.thread_data;
...
void *qemu_thread_join(QemuThread *thread)
{
    ...
    if (handle) {
        ...
        GetExitCodeThread(handle, (LPDWORD)&ret);
        CloseHandle(handle);
    }
    return ret;
}
...
void qemu_thread_register(QemuThread *thread, const char *name)
{
    ...
    data->exited = false;
    notifier_list_init(&data->exit);
    current_thread->base.thread_data = data;
